...
class Simulator {
	...
	public static void main(String[] args)
	{
		...
		if (game_id < 0) System.exit(0);
		// bubblesort by score
		int[] id = new int [] {0, 1, 2, 3};
		for (int i = 0 ; i != 4 ; ++i)
			for (int j = 1 ; j != 4 ; ++j)
				if (score[id[j - 1]] < score[id[j]]) {
					int t = id[j];
					id[j] = id[j - 1];
					id[j - 1] = t;
				}
		// order including ties
		String[] place = new String [] {"1st", "2nd", "3rd", "4th"};
		for (int i = 1 ; i != 4 ; ++i)
			if (score[id[i]] == score[id[i - 1]])
				place[i] = place[i - 1];
		// store scores
		try {
			FileOutputStream play_file = new FileOutputStream(play_path, true);
			FileOutputStream game_file = new FileOutputStream(game_path, true);
			for (int i = 0 ; i != 4 ; ++i) {
				int j = id[i];
				String timeout = players[i] == null ? "yes" : "no";
				String entry = "" + game_id + "," + groups[j] + "," + score[j] +
				                              "," + place[i]  + "," + timeout + "\n";
				play_file.write(entry.getBytes());
			}
			String entry = game_id + "," + n_pipers + "," + n_rats
			                       + "," + inner_side + "\n";
			game_file.write(entry.getBytes());
			play_file.close();
			game_file.close();
		} catch (IOException e) {
			System.err.println("Error during CSV write: " + e.getMessage());
			System.exit(1);
		}
		System.exit(0);
	}
