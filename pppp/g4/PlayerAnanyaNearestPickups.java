...
public class PlayerAnanyaNearestPickups implements pppp.sim.Player {
	...
	public void init(int id, int side, long turns,
		...
		for (int p = 0; p != n_pipers; ++p) {
            ...
            */
			...
			// List<Point> val = get_rats_boundary(rats, box_boundaries);
   //          for(Point rat : val)
   //          {
   ...
   //          }
    ...
    public Map<Integer, Point> get_piper_to_cell(int remaining_pipers) {
        ...
        // System.out.println("Calling get piper to cell method !!!! ");
        ...
        try {
            ...
            for (int p = 0; p != pipers[id].length; ++p) {
                ...
                if (dst!=null && Math.abs(src.x - dst.x) < 0.000001 &&
                    ...
                    // get next position
                    // System.out.println("Piper no : " + p + " | Position reached ---> " + dst.x + " | " + dst.y);
                    ...
                    if (dst == null || pos_index[p] == 5) {
                        ...
                        // System.out.println(" ---> For piper no : " + p + " Dst obtained : x " +piper_to_cell.get(p).x + " | y " + piper_to_cell.get(p).y);
            ...
            System.out.println("5");
                ...
                if (pos_index[p] == 6 && num_captured_rats(pipers[id][p], rats) == 0) {
                    ...
                    // sort the cells in the Cell[] grid in descending order of weight/number_of_rats
                    Arrays.sort(this.grid, Collections.reverseOrder());
                    // System.out.println(" ---> For piper no : " + p);
                    piper_to_cell = get_piper_to_cell(pipers[id].length);
                }
            ...
            System.out.println("7");
                ...
                // if pos_index = 6 and the closest rat is not null. then make it 5 and set dst to closest rat index
                // around here print piper + rat index
                ...
                {            System.out.println("8");
                   ...
                   // System.out.println(" ---> For piper no : " + p);
                    System.out.println("piper no : "+ p + "In pos index 6, nrats > 1 "  + pos_index[p] + " | isBrat flag " + isBoundaryRat[p]);
                    pos_index[p] = 5;
