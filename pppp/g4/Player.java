...
public class Player implements pppp.sim.Player {
     ...
     */
        ...
        for(int i=0; i < dim; i++) {
            for(int j=0; j < dim; j++) {
                Cell cell = new Cell(
                                cell_side,//size of CELL's 1 Border
	...
	private void playForNomalRun(Point[][] pipers, boolean[][] pipers_played,
			 Point[] rats, Move[] moves){
        ...
        try {
            ...
            for (int p = 0; p != pipers[id].length; ++p) {
                ...
                if (pos_index[p] == 6 && Utils.num_captured_rats(pipers[id][p], rats) == 0) {
                    pos_index[p] = 5;
                    grid = create_grid(side, rats.length);
                    update_grid_weights(rats, pipers, our_gate);
                    // sort the cells in the Cell[] grid in descending order of weight/number_of_rats
	        ...
	        try {
	            ...
	            for (int p = 0; p != pipers[id].length; ++p) {
	            	...
	            	int capturedRats= Utils.num_captured_rats(pipers[id][p], rats);
	            		...
	            		if(pos_index[p]==0 && Utils.reachedDst(src,pos[p][pos_index[p]])){
	            			pos_index[p]=2;
	            		}else if(pos_index[p]==2 && Utils.reachedDst(src,pos[p][pos_index[p]])){
	            			pos_index[p]=5;
	            			
	            		}else if(pos_index[p]==5 && Utils.reachedDst(src,pos[p][pos_index[p]]) && capturedRats >20){
	            			pos_index[p]=6;
	            		}
	            		else if (pos_index[p]==6 && Utils.reachedDst(src,pos[p][pos_index[p]])){
	            			pos_index[p]=7;
	            		}else if (pos_index[p]==7 && Utils.reachedDst(src,pos[p][pos_index[p]])){
	            			pos_index[p]=0;
	            		}
	            	...
	            	if(pos_index[p] ==0 ){
	            		...
	            		if(whichCurrentCluster[p]==-1  ||
		            		...
		            		while(dst==null ||  Utils.getRatsCountOnDst( rats, dst,nearbyRatScanRadius) ==0){
			            		DenseCell d=Utils.getInitialConfigForMaxRatCluster(rats, clusters, whichCurrentCluster[p], our_gate, 
			            				((dst==null || whichCurrentCluster[p]==-1  )? ((isSparse(rats.length, side))?side : side*.8 ): maxDistanceInCurrentCluster[p] *.6)) ;//NEED TO capture the most dense cluster in the cluster as start point.
			            		if(d==null){
			            			reachedFirstPoint[p]=false;
		            		...
		            		if(capturedRats<1 &&  Utils.getRatsCountOnDst( rats, src,nearbyRatScanRadius) ==0){
			            		reachedFirstPoint[p]=false;
		            		...
		            		continue;
		            			...
		            			reachedFirstPoint[p] && 
		            			Utils.ratsOnPath( rats , clusters.get(whichCurrentCluster[p]).getPoints(), nearbyRatScanRadius,posIndexInCurrentCluster[p]) ==0 )||
		            			Utils.getRatsCountOnDst( rats, dst,nearbyRatScanRadius) ==0 ||
		            					Utils.getRatsCountOnDst( rats, pos[p][pos_index[p]],nearbyRatScanRadius) ==0 ){
		            		...
		            		while(dst==null ||  Utils.getRatsCountOnDst( rats, dst,nearbyRatScanRadius) ==0){
		            			updateClusters(rats);
			            		DenseCell d=Utils.getInitialConfigForMaxRatCluster(rats, clusters, whichCurrentCluster[p], our_gate, 
			            				((dst==null ||Utils.getRatsCountOnDst( rats, pos[p][pos_index[p]],nearbyRatScanRadius) ==0 ) ? ((isSparse(rats.length, side))?side : side*.8 ) : maxDistanceInCurrentCluster[p] *.6)) ;//NEED TO capture the most dense cluster in the cluster as start point.
			            		if(d==null){
			            			reachedFirstPoint[p]=false;
		            				whichCurrentCluster[p]=-1;
			            			dst=pos[p][6];//next call will reset its position
	            			...
	            			//until we find the correct DST
		            		while(dst==null ||  Utils.getRatsCountOnDst( rats, dst,nearbyRatScanRadius) ==0 || Utils.reachedDst(src,pos[p][pos_index[p]])){
		            			DenseCell d=null;
		            			if(whichCurrentCluster[p] ==-1){
			            			reachedFirstPoint[p]=false;
		            				whichCurrentCluster[p]=-1;
			            			...
			            			d=Utils.getInitialConfigForMaxRatCluster(rats, clusters, whichCurrentCluster[p], our_gate, 
			            				(dst==null  ? ( (isSparse(rats.length, side))?side : side*.8 ) : maxDistanceInCurrentCluster[p] *.6)) ;//NEED TO capture the most dense cluster in the cluster as start point.
			            		}
			            		if(d==null){
			            			reachedFirstPoint[p]=false;
		            				whichCurrentCluster[p]=-1;
